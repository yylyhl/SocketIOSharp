# New
This is a fork of [ SocketIOSharp:master ](https://github.com/uhm0311/SocketIOSharp) with additional support for socket connection pre-processing!
```csharp
...
# SocketIOSharp
`SocketIOSharp` is a **Socket.IO protocol revision `4`** library based on `Engine.IO` and `WebSocket` protocol. It depends on [EngineIOSharp](https://github.com/yylyhldev/EngineIOSharp) to use `Engine.IO` protocol.

# Installation
- [Nuget gallery](https://www.nuget.org/packages/SocketIOSharp2)

- Command `Install-Package SocketIOSharp2` in nuget package manager console.
